...
| `agent-spec atlas benchmark validate/plan/summarize` | Validate a corpus, compile paired Atlas/baseline run plans, and summarize fully graded JSON-array or NDJSON receipts; `plan` and `summarize` write atomically with `--out` and otherwise print JSON | Evaluate correctness before performance; every corpus arm has at least three trials. The opt-in runner is `scripts/atlas-eval/run-opt-in.sh`, requires `jq`, complete typed receipts for every planned run, and `ATLAS_EVAL_AGENT_COMMAND` as one external executable, never a shell builtin/function or command string. See `docs/atlas-evaluation.md` |
...
| `agent-spec requirements export --out requirements.yaml [--dialect arc-native --root-name N]` | YAML projection of confirmed requirements (round-trip fixpoint, `--check` drift gate); `arc-native` emits the reference compiler's single-root input tree with dotted-id restoration | Interop with YAML-world tooling — including feeding compiled requirements straight to ARC; derived, never source of truth |
