...
> 前言 → 第 1 章 → 第 2 章 → 第 3 章 → 第 4 章 → 第 5 章 → 第 7 章 → 附录 A

读完你能独立写合同、跑验证、看懂五种 verdict，并把 guard 挂进 CI。

**路径 B：架构师深读**（评估它是否值得引入团队）

> 前言 → 第 1 章 → 第 18 章 → 第 19 章 → 第 10 章 → 第 11 章 → 第 13 章 → 附录 D

先看哲学与架构再回到编译管线，最后用两条端到端轨迹验证叙事的完整性。
...
- 命令块中的输出都来自 **真实运行**（agent-spec 1.0.0），未经虚构。
- `file:line` 形式的引用指向 agent-spec 仓库源码。
- "详见第 N 章" 是全书统一的交叉引用格式。
- 每章开篇的 `> **定位**` 告诉你这一章讲什么、依赖哪一章、什么时候需要读它。
