...
| 查询质量回归仍偏 fixture 级 | 缺少固定真实仓库 revision、golden symbol/path 与失败归因时，ranking 或压缩调整可能在单测通过时降低 Agent 答案质量 |
...
状态：下一批优先项；为 B5 和后续 A4 mechanism 提供晋升门。
...
| 9 | E3 query quality regression loop | E0、B2/B3/B4 | 先给后续 ranking、压缩和 dynamic mechanism 建立晋升门 |
