...
状态：Wave 2 accepted contract，尚未交付。
...
状态：纳入 Wave 2 accepted contract，尚未交付。
...
| 4 | B2/B3 explore 与 flow | E0、A2、B1、D1 | 给 Agent 一个内容充分的架构查询并延续离线评测契约 |
| 5 | B4 impact 与 affected code | E0、B1、B3 | 提供确定性反向遍历与同一 receipt 指标 |
...
该 requirement 已为 `accepted`，但在 Contract lifecycle、requirement replay 和真实代码证据
全部通过前，B2/B3/B4 仍保持“尚未交付”。E1 本轮只增加 response bytes、read-back、后续
查询与 truncation 的离线 receipt 指标；默认 MCP surface 的变化仍必须等待 opt-in 真实 A/B。
