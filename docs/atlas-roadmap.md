...
> B2/B3/B4、A3、A4 trait v1、A4.1、D2、E3，以及 Track C 的 C1/C2/C3 已交付；后续
...
| 查询输出仍只有固定 compact/deep projection | retrieval miss 与 context truncation 不能分开归因，也没有稳定 omission/continuation 合约 |
...
| 尚无可选 watcher/daemon 与 pending watermark | D2 已移除健康零变更的全图地板，但自动同步、retry 和 degraded lifecycle 仍属于 D3 |
| 尚无并发查询 backpressure 与 transport isolation | 多 Agent 的 CPU-heavy traversal 可能让共享 daemon 的 MCP control/status 失去响应 |
...
- **D3.7 Safe reclamation**：只有 single-writer identity 与 reader retention/lease 足以证明路径
...
状态：未来工作；依赖 D3 稳定的 snapshot lease。B5 load profile 之后可以交付 opt-in
prototype，是否默认启用由 E1 的并发负载数据决定。

CodeGraph 的 query worker pool 说明：共享 daemon 即使图已新鲜，CPU-heavy traversal 和 source
projection 仍可能阻塞 MCP transport。Atlas 不预设必须使用线程池，而是先定义可测量的服务
契约：
...
启用。单 Agent、小仓库和 CI 必须保留零 worker 的直接路径。
...
供下一次 build 恢复；D3 watcher、daemon、retry 与 pending watermark 仍未启用。
