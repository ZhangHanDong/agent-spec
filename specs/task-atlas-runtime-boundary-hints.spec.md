...
场景: generic inherent callback 通过 impl graph 解析
  测试:
    过滤: test_runtime_boundary_generic_inherent_candidate_uses_impl_edges
    层级: unit
  假设 `Handler::<u8>::callback` 对应 generic inherent implementation method
  当 query index 解析 preserved callback path
  那么 candidate id 精确等于 `callback` 且 lookup 使用 `ImplFor` 与 `Contains` relations
...
场景: frontier budget 前全局排序每个 breadth layer
  测试:
    过滤: test_runtime_boundary_scan_frontier_orders_each_depth_before_limits
    层级: unit
  假设 parent `a` 的 children 为 `z-*` 且 parent `b` 的 children 为 `a-*`
  当 8-function limit 截断同一个 depth layer
  那么 depth-two prefix 精确等于 `a-0` 至 `a-4`
